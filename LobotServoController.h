...
extern  int shakeHeadActionGroup[7];//转头动作组标号

struct LobotServo {  //舵机ID和位置数据结构
  uint8_t  ID;       //舵机ID
...
class LobotServoController {
  public:
    ...
    static void moveServo(uint8_t servoID, uint16_t Position, uint16_t Time);
    static void moveServos(LobotServo servos[], uint8_t Num, uint16_t Time);
    static void moveServos(uint8_t Num,uint16_t Time, ...);
    static void runActionGroup(uint8_t NumOfAction, uint16_t Times);
    static void stopActiongGroup(void);
    static void setActionGroupSpeed(uint8_t NumOfAction, uint16_t Speed);
    static void setAllActionGroupSpeed(uint16_t Speed);

	static void run(action act);
	static void toPose(int pose);//指向对应的方向
