...
int shakeHeadActionGroup[7] = { 13,12,11,14,15,16,17 };//转头动作组标号
...
void LobotServoController::moveServos(LobotServo servos[], uint8_t Num, uint16_t Time)
{
	...
	Serial.write(buf, buf[2] + 2); //发送帧，长度为数据长度+两个字节的帧头
}
 ...
 * Others:
 **********************************************************************************/
void LobotServoController::stopActiongGroup(void)
{
	uint8_t buf[4];
	buf[0] = FRAME_HEADER;     //填充帧头
	buf[1] = FRAME_HEADER;
...
void LobotServoController::receiveHandle()
{
	...
	case 2:
		...
		break;
	case 3:
		if (Serial.readBytes(buf, len - 1) > 0)
			getHeader = 4;
		else{
			len = 0;
			getHeader = 0;
...
void LobotServoController::run(action act) {
	switch (act)
	{
	case putUp:
		runActionGroup(0, 1);
...
void LobotServoController::toPose(int pose)
{
	runActionGroup(shakeHeadActionGroup[pose / 30], 1);
	delay(300);
}
